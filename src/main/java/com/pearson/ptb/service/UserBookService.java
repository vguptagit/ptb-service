...
public class UserBookService {
	...
	public void importUserBooks(List<String> userBookIds, String userId) {
		...
		userQuestionsfolderSeq = userQuestionsfolderSeq
				/ Math.pow(size, userBookIds.size());

		String myQuestionsFolderId = userFolderService
				.getMyQuestionsFolder(userId).getGuid();
		...
		for (String userBookId : userBookIds) {
			...
			if (!userBook.getIsImported()) {
				...
				if (!userBook.getTestBindings().isEmpty()) {
					userFolderService.saveFolder(
							getUserFolder(userBook, folderSeq), userId);
				}

				if (!userBook.getQuestionBindings().isEmpty()) {
					userFolderService.saveFolder(getUserQuestionsFolder(
							userBook, userQuestionsfolderSeq,
							myQuestionsFolderId), userId);
				}
	...
	private UserQuestionsFolder getUserQuestionsFolder(UserBook userBook,
			double folderSeq, String parentId) {
