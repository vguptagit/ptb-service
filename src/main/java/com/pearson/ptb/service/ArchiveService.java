...
/**
 * This <code>ArchiveService</code> is responsible for archiving the folders and tests.
 *
 */
...
public class ArchiveService {
	...
	public ArchivedFolder archiveFolder(String folderId) {	
		...
		ArchivedFolder archivedFolder = parentsAndSelf.get(parentsAndSelf.size() - 1);
	...
	public void deleteTest(String folderId, String testId, String userId) {	
		...
		for(TestBinding testBinding : testBindings) {
			...
			if(testBinding.getTestId().equals(testId)) {
				...
				// Delete test from PAF
				myTestsRepo.delete(testBinding.getTestId());
				CACHE.delete(String.format(CacheKey.TEST_FORMAT, testBinding.getTestId()));
			}
			index++;
		}
	 ...
	 * @return ArchivedFolder.
	 */
	public ArchivedFolder archiveTest(String userId, String testId, String folderId) {		
		...
		if(archivedSelfFolder.getUserID().equals(archivedSelfFolder.getParentId())) {
	 ...
	 * @return UserFolder.
	 */
	public UserFolder restoreTest(String userId, String testId, String folderId) {				
		loggedInUser=userId;
	...
	private List<UserFolder> userParentsAndSelf(String folderId) {
		...
		// The below block of code will generate the parent (folders) hierarchy to be archived.
		List<ArchivedFolder> archivedFolders = new ArrayList<ArchivedFolder>();
		fillArchivedParentFolders(archivedFolders, folderId);	
				
		RestoredAndUnRestored restoredAndUnRestored = generateUserParentFolders(archivedFolders, null);
		
		if(!restoredAndUnRestored.alreadyRestoredFolders.isEmpty()){
			userFoldersRepo.saveFolders(restoredAndUnRestored.alreadyRestoredFolders);
		}
	...
	private List<ArchivedFolder> archiveParentsAndSelf(String folderId) {
		...
		ArchivedAndUnArchived archivedAndUnArchived = generateArchivedParentFolders(userFolders, null);
	...
	private void archiveChildren(String folderId, String archivedFolderId) {
		...
		// The below block of code will generate the child (folders and tests) hierarchy to be archived.
		List<String> folderIds = new ArrayList<>(Arrays.asList(folderId));		
		List<UserFolder> userFolders = new ArrayList<UserFolder>();
		
		UserFolder userFolderSelf = userFoldersRepo.getFolder(folderId);
		ArchivedFolder archivedFolderSelf = archiveRepo.getFolder(archivedFolderId);
		...
		if(!userFolders.isEmpty()) {
			ArchivedAndUnArchived archivedAndUnArchived = generateArchivedChildFolders(userFolders, archivedFolderId);
			if(!archivedAndUnArchived.unArchivedFolders.isEmpty()) {
				archiveRepo.archiveFolders(archivedAndUnArchived.unArchivedFolders);
			}
			if(!archivedAndUnArchived.alreadyArchivedFolders.isEmpty()) {
				archiveRepo.archiveFolders(archivedAndUnArchived.alreadyArchivedFolders);
			}
		}

		// The below block of code will delete the child folders that are already archived
	...
	private void restoreChildren(String folderId, String userFolderId) {
		...
		if(!archivedFolders.isEmpty()) {
			RestoredAndUnRestored restoredAndUnRestored = generateRestoredChildFolders(archivedFolders, userFolderId);
			if(!restoredAndUnRestored.unRestoredFolders.isEmpty()) {
				userFoldersRepo.saveFolders(restoredAndUnRestored.unRestoredFolders);	
			}
			if(!restoredAndUnRestored.alreadyRestoredFolders.isEmpty()) {
				userFoldersRepo.saveFolders(restoredAndUnRestored.alreadyRestoredFolders);	
			}
	...
	private ArchivedAndUnArchived generateArchivedParentFolders(List<UserFolder> userFolders, String intialParentId) {
		...
		for(UserFolder userFolder : userFolders) {			
			...
			if(isNotArchived(userFolder.getTitle(), parentId, userFolder.getUserID())) {
				...
				alreadyArchivedFolder = archiveRepo.getFolder(userFolder.getTitle(), parentId, userFolder.getUserID());
	...
	private RestoredAndUnRestored generateUserParentFolders(List<ArchivedFolder> archivedFolders, String intialParentId) {
		...
		for(ArchivedFolder archivedFolder : archivedFolders) {			
			...
			if(isNotRestored(archivedFolder.getTitle(), parentId, archivedFolder.getUserID())) {
				...
				if(folderCount==archivedFolders.size()){
					if(restoreType==RestoreType.TEST){
						...
						unRestoredFolder.setTestBindings(archivedFolder.getTestBindings());	
					}	
				}else{
					List<TestBinding>  testBindings= new ArrayList<TestBinding>();
					unRestoredFolder.setTestBindings(testBindings);
				...
				alreadyRestoredFolder = userFoldersRepo.getFolderByTitle(archivedFolder.getTitle(), parentId, archivedFolder.getUserID());
	...
	private void checkDuplicateAndAdd(UserFolder alreadyRestoredFolder, ArchivedFolder  archivedFolder){
		if(!alreadyRestoredFolder.getTestBindings().isEmpty()){
			List<TestMetadata> userTests = myTestService.getMyFolderTests(loggedInUser, alreadyRestoredFolder.getGuid(), false);
			List<TestMetadata> archivedTests =getMyFolderTests(archivedFolder);
			
			if(restoreType==RestoreType.TEST){
				for(TestMetadata userTest: userTests){
					if(restoringTestMetadata.getTitle().equals(userTest.getTitle())){
						throw new DuplicateTitleException(DuplicateTitleMessage);
					}
				}
			}else if(restoreType==RestoreType.FOLDER){
				for(TestMetadata userTest: userTests){
					for(TestMetadata archivedTest: archivedTests){
						if(userTest.getTitle().equals(archivedTest.getTitle())){
							throw new DuplicateTitleException(DuplicateTitleMessage);
						}
					}
				}
		...
		if(!archivedFolder.getTestBindings().isEmpty()){
			if(restoreType==RestoreType.TEST){
				if(alreadyRestoredFolder.getTestBindings().isEmpty()){
					...
					alreadyRestoredFolder.getTestBindings().add(restoringTestBinding);	
				}
			}else if(restoreType==RestoreType.FOLDER){
				if(alreadyRestoredFolder.getTestBindings().isEmpty()){
					alreadyRestoredFolder.setTestBindings(archivedFolder.getTestBindings());
				}else{
					alreadyRestoredFolder.getTestBindings().addAll(archivedFolder.getTestBindings());	
				}	
			}
		}
	}
	
	private ArchivedAndUnArchived generateArchivedChildFolders(List<UserFolder> userFolders, String intialParentId) {
		...
		for(UserFolder userFolder : userFolders) {			
			...
			if(isNotArchived(userFolder.getTitle(), parentId, userFolder.getUserID())) {
				...
				parentIdMap.put(userFolder.getGuid(), unArchivedFolder.getGuid());
	...
	private RestoredAndUnRestored generateRestoredChildFolders(List<ArchivedFolder> archivedFolders, String intialParentId) {
		...
		for(ArchivedFolder archivedFolder : archivedFolders) {			
			if(isNotRestored(archivedFolder.getTitle(), parentId, archivedFolder.getUserID())) {
				...
				if(parentIdMap.containsKey(archivedFolder.getParentId())){
					unRestoredFolder.setParentId(parentIdMap.get(archivedFolder.getParentId()));	
				}else{
					unRestoredFolder.setParentId(parentId);
				...
				parentId = alreadyRestoredFolder.getGuid();
				if(!parentIdMap.containsKey(archivedFolder.getParentId())){
					parentIdMap.put(archivedFolder.getParentId(), alreadyRestoredFolder.getParentId());
				}
	...
	private boolean isNotArchived(String folderTitle, String archivedParentId, String userId) {
				 
		ArchivedFolder folder = archiveRepo.getFolder(folderTitle, archivedParentId, userId);
		
		return folder == null;
	}
	
	private boolean isNotRestored(String folderTitle, String restoredParentId, String userId) {
		 
		UserFolder folder = userFoldersRepo.getFolderByTitle(folderTitle, restoredParentId, userId);
		
		return folder == null;
	}
	
	private void fillParentFolders(List<UserFolder> userFolders, String folderId) {
	...
	private void fillArchivedParentFolders(List<ArchivedFolder> archivedFolders, String folderId) {
		ArchivedFolder archivedFolder = archiveRepo.getFolder(folderId);
		
		if(archivedFolder.getParentId() != null) {
			fillArchivedParentFolders(archivedFolders, archivedFolder.getParentId());
		}
		
		archivedFolders.add(archivedFolder);
	}
	
	private void fillChildFolders(List<String> folderIds, List<UserFolder> userFolders, String folderId) {
	...
	private void fillArchivedChildFolders(List<String> folderIds, List<ArchivedFolder> archivedFolders, String folderId) {
		...
		for(ArchivedFolder child : children) {
			...
			folderIds.add(child.getGuid());
			fillArchivedChildFolders(folderIds, archivedFolders, child.getGuid());			
		}
	}
