...
/**
 * This <code>BlackboardEssay</code> is responsible for support black board format for essay questions.
 */
public class BlackboardEssay extends BlackboardQTIConvert {
	 ...
	 */
	public BlackboardEssay(DownloadFormat  bbFormat,int sequence, String ptbQuestionXML) {
	...
	private void updateResprocessing() {
		try {
			...
			resprocessingMaxvalue.getAttributes().getNamedItem("maxvalue").setNodeValue(ptbQTIParser.getMaximumScore());
	...
	private void updateItemfeedback() {
		try {
			XPathExpression expr=xpath.compile("//itemfeedback[@ident=\"solution\"]");
			Node recommendedAnswer=(Node)expr.evaluate(bbQuestionXML, XPathConstants.NODE);
			((Element)recommendedAnswer).getElementsByTagName("mat_formattedtext").item(0).appendChild(bbQuestionXML.createCDATASection(ptbQTIParser.getRecommendedAnswer()));
		} catch (Exception e) {
			throw new InternalException(
