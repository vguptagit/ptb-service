...
/**
 * This <code>BlackboardMatching</code> is responsible for support black board format for matching questions.
 */
public class BlackboardMatching extends BlackboardQTIConvert {
	...
	/*
	 * Abstract method of super class is implemented. This will return the blackboard question XML
	 * @see com.pearson.mytest.download.BlackboardQTIConvert#getBlackboardQuestionXML()
	 */
	@Override
	...
	private void createResponseBlock(Multimap<String, String> answerChoices){
		...
		Node flowBlock = ((Element)flowResponseBlock).getElementsByTagName("flow").item(0);
		
		int i = 1;
		for (Map.Entry<String, String> entry : answerChoices.entries()) {
			if (i == 1) {
				((Element)flowBlock).getElementsByTagName(MAT_FORMATTEDTEXT).item(0).appendChild(bbQuestionXML.createCDATASection(entry.getKey()));
				createResponseLabel(((Element)flowBlock).getElementsByTagName(RESPONSE_LID).item(0),i,answerChoices.size());
				
			} else {
				Node newFlowBlock=flowBlock.cloneNode(true);
				Node matTestNode=((Element)newFlowBlock).getElementsByTagName(MAT_FORMATTEDTEXT).item(0);
				removeAllChild(matTestNode);
				matTestNode.appendChild(bbQuestionXML.createCDATASection(entry.getKey()));
				createResponseLabel(((Element)newFlowBlock).getElementsByTagName(RESPONSE_LID).item(0),i,answerChoices.size());
	...
	private void createResponseLabel(Node responseLid, int index, int answerChoiceLength){
		responseLid.getAttributes().getNamedItem("ident").setNodeValue(Integer.toString(index));
		Node responseLabel=((Element)responseLid).getElementsByTagName("response_label").item(0);
		Node flowLabel=((Element)responseLid).getElementsByTagName("flow_label").item(0);
		...
		for (int i=0;i<answerChoiceLength; i++) {
			...
			newResponseLabel.getAttributes().getNamedItem("ident").setNodeValue(chart+Integer.toString(i+1));
	...
	private void createRightMatchBlock(Multimap<String, String> answerChoices) {
		...
		Node flowBlock = ((Element)flowRightMatchBlock).getElementsByTagName("flow").item(0);
		
		int i = 1;
		for (Map.Entry<String, String> entry : answerChoices.entries()) {
			if (i == 1) {
				((Element)flowBlock).getElementsByTagName(MAT_FORMATTEDTEXT).item(0).appendChild(bbQuestionXML.createCDATASection(entry.getValue()));
			}else{
				Node newFlowBlock= flowBlock.cloneNode(true);
				Node matTestNode=((Element)newFlowBlock).getElementsByTagName(MAT_FORMATTEDTEXT).item(0);
				removeAllChild(matTestNode);
				matTestNode.appendChild(bbQuestionXML.createCDATASection(entry.getValue()));
	...
	private void updateResprocessing(Multimap<String, String> answerChoices) {
		Node resprocessing=getXMLNode(RESPROCESSING_SCOREMODEL_SUMOFSCORES,true);
		Node resprocessingMaxvalue = getXMLNode(RESPCONDITION_MAXVALUE,true);
		resprocessingMaxvalue.getAttributes().getNamedItem("maxvalue").setNodeValue(this.getSumScore());
		Node respCondition=getXMLNode("respcondition",false);
		Node respIncorrectCondition=getXMLNode(RESPROCESSING_RESPCONDITION_INCORRECTTITLE,true);
		int i=0;
		for (Map.Entry<String, String> entryShuffled : answerChoices.entries()) {
			String chart=Character.toString((char)(ASCII_CHAR+i))+getCorrectAnswerIndex(entryShuffled.getKey(),answerChoices);
			if (i == 0) {
				Node varequal=((Element)respCondition).getElementsByTagName("varequal").item(0);
				varequal.getAttributes().getNamedItem("respident").setNodeValue(Integer.toString(i+1));
				varequal.setTextContent(chart);
			}else{
				Node newRespCondition=respCondition.cloneNode(true);
				Node varequal=((Element)newRespCondition).getElementsByTagName("varequal").item(0);
				varequal.getAttributes().getNamedItem("respident").setNodeValue(Integer.toString(i+1));
				varequal.setTextContent(chart);
				resprocessing.insertBefore(newRespCondition,respIncorrectCondition);
			}
			i++;
		}
	}
	
	private String getCorrectAnswerIndex(String leftAnswerChoice, Multimap<String, String> answerChoices){
		Multimap<String, String> answerChoicesUnShuffled=ptbQTIParser.getAnswerChoicesForMatching(false);
		String rightAnswerChoice="";
		
		for (Map.Entry<String, String> entryUnShuffled : answerChoicesUnShuffled.entries()) {
