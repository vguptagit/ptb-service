...
/**
 * This <code>BlackboardFillInBlank</code> is responsible for support black board format for fill in the blank questions.
 */
public class BlackboardFillInBlank extends BlackboardQTIConvert {
	 ...
	 */
	public BlackboardFillInBlank(DownloadFormat  bbFormat,int sequence, String ptbQuestionXML) {
	...
	private void updateResprocessing() {
		...
		Node respconditionIncorrect=getXMLNode(RESPROCESSING_RESPCONDITION_INCORRECTTITLE,true);
		
		
		int i=1;
        for (Entry<String, Double> entry : answerKeysWithScore.entries()) {
           if(i==1){
        	   respcondition.getAttributes().getNamedItem(TITLE).setNodeValue(RESPONSE+i);
        	   Node varequal=((Element)respcondition).getElementsByTagName("varequal").item(0);
        	   varequal.getAttributes().getNamedItem("respident").setNodeValue(Integer.toString(i));
        	   varequal.setTextContent(entry.getKey());
           }else{
        	   Node newRespCondition = respcondition.cloneNode(true);
        	   
        	   newRespCondition.getAttributes().getNamedItem(TITLE).setNodeValue(RESPONSE+i);
        	   Node varequal=((Element)newRespCondition).getElementsByTagName("varequal").item(0);
        	   varequal.getAttributes().getNamedItem("respident").setNodeValue(Integer.toString(i));
        	   varequal.setTextContent(entry.getKey());
        	   renderChoice.insertBefore(newRespCondition,respconditionIncorrect);  
           }
        	i=i+1;
        }  
	...
	private void updateItemfeedback() {
		...
		for (int i=0;i<getAnswerChoiceLength(); i++) {
			if (i == 0) {
				itemfeedback.getAttributes().getNamedItem("ident").setNodeValue(RESPONSE+(i+1));
			}else{
				Node newItemfeedback=itemfeedback.cloneNode(true);
				newItemfeedback.getAttributes().getNamedItem("ident").setNodeValue(RESPONSE+(i+1));
