...
/**
 * This <code>BlackboardTrueFalse</code> is responsible for support black board format for true false questions.
 */
public class BlackboardTrueFalse extends BlackboardQTIConvert {
	 ...
	 */
	public BlackboardTrueFalse(DownloadFormat  bbFormat,int sequence, String ptbQuestionXML) {
	...
	private void updateAnswerChoices(){
		try {
			...
			for (int i = 0; i < responselabels.getLength(); i++) {		
				...
				String key = currentItem.getAttributes().getNamedItem("ident").getNodeValue();
			
				if(IDENT_1.equals(key)) {					
					((Element)currentItem).getElementsByTagName(MATTEXT).item(0).setTextContent(answerChoicesList.get(i).toString());
		...
		try {
			...
			for (int i = 0; i < respconditions.getLength(); i++) {		
				...
				if("correct".equals(key)) {
					((Element)currentItem).getElementsByTagName("varequal").item(0).setTextContent(ptbQTIParser.getAnswerKeys());
				} 
			}
		} catch (Exception e) {
			throw new InternalException(
	...
	private void updateItemfeedback() {
		...
		try {
			NodeList responseLabels =	getXMLNodes("response_label",false);
			itemfeedbacks = ((NodeList) xpath.evaluate(ITEMFEEDBACK, bbQuestionXML, XPathConstants.NODESET));
			for (int i = 0; i < itemfeedbacks.getLength(); i++) {		
				Node currentItem = itemfeedbacks.item(i);
				String answerChoice=((Element)responseLabels.item(i)).getElementsByTagName("mattext").item(0).getTextContent();
				((Element)currentItem).getElementsByTagName("mat_formattedtext").item(0).appendChild(bbQuestionXML.createCDATASection(answerChoiceFeedback(answerChoice)));
			}
		} catch (Exception e) {
			throw new InternalException(
	...
	private String answerChoiceFeedback(String answer){
		Map<String, String> answerChoicesWithFeedback = ptbQTIParser.getAnswerChoicesWithFeedback();
		String feedback="";
		for(Entry<String, String> entry : answerChoicesWithFeedback.entrySet()){
