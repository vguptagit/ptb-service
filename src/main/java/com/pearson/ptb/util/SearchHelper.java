...
public class SearchHelper {
	...
	public static void updateSearchValues(Map<String, String> filterCriteria) {
		...
		for (Map.Entry<String, String> entry : filterCriteria.entrySet()) {
			if (entry.getKey()
					.equals(QuestionSearchCriteria.quizType.toString())) {
				...
				entry.setValue(value);
					...
					.equals(QuestionSearchCriteria.questionLevelOfDifficulty
							.toString())) {
				String value = entry.getValue();
	...
	public static Map<String, String> getQuestionFilterCriteriaMap() {
		if (searchCriteriaMap == null) {
			...
			searchCriteriaMap.put("QTyp",
					QuestionSearchCriteria.quizType.toString());
			searchCriteriaMap.put("Diff",
					QuestionSearchCriteria.questionLevelOfDifficulty
							.toString());
			searchCriteriaMap.put("Topk",
					QuestionSearchCriteria.topic.toString());
			searchCriteriaMap.put("Objt",
					QuestionSearchCriteria.objective.toString());
			searchCriteriaMap.put("PRef",
					QuestionSearchCriteria.pageReference.toString());
			searchCriteriaMap.put("Skil",
					QuestionSearchCriteria.skill.toString());
			searchCriteriaMap.put("QnId",
					QuestionSearchCriteria.questionId.toString());
		}
	...
	public static void filterQuestionBySearchCriteria(
			...
			List<QuestionMetadata> filteredQuestions) {
		for (Map.Entry<String, String> entry : filterCriteria.entrySet()) {
			if (entry.getKey()
					.equals(QuestionSearchCriteria.quizType.toString())) {
				List<String> quizTypeList = Arrays
						.asList(entry.getValue().split(","));
				if (!quizTypeList.contains(questionMetadata.getQuizType())) {
					return;
				}
			} else if (entry.getKey()
					.equals(QuestionSearchCriteria.questionId.toString())) {
				if (!questionMetadata.getGuid().equals(entry.getValue())) {
					return;
				}
			} else {
				ExtMetadata qMetadata = questionMetadata
						.getExtendedMetadataByName(entry.getKey());
				if (qMetadata == null) {
					return;
				} else {
					...
					if (entry.getKey().equals(
							QuestionSearchCriteria.questionLevelOfDifficulty
									.toString())) {
						List<String> difficultyLevel = Arrays
								.asList(entry.getValue().split(","));
						if (!difficultyLevel.contains(metadataValue)) {
							return;
						}
					} else if (!StringUtils.containsIgnoreCase(metadataValue,
							entry.getValue())) {
						return;
					}
				}
